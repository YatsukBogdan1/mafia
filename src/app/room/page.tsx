...
type RoleDistribution = import('@/lib/game/types').RoleDistribution;
...
function RoomSettingsModal({
  ...
  settings: import('@/lib/game/types').RoomSettings;
  playerCount: number;
  onSave: (s: Partial<import('@/lib/game/types').RoomSettings>) => void;
  onClose: () => void;
}) {
...
function HostControls({
  ...
  const { phase, players, vote, speaking, round, playerOrder, settings } = gameState;
    ...
    <>
      ...
      {inGame && (
        <>
          ...
          {playerOrder.length > 0 && players[playerOrder[0]]?.role === null && (
